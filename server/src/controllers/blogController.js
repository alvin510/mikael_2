  ...
  async (req, res) => {
    try {
      ...
      if (req.files["gambar"] && req.files["gambar"][0]) {
        ...
        res
          .status(200)
          .json({ blog: savedBlog, message: "Blog telah ditambahkan." });
      } else {
...
route.post(
  ...
  async (req, res) => {
    try {
      ...
      res
        .status(200)
        .json({ blog: updatedBlog, message: "Blog telah diubah." });
    } catch (error) {
      // Tangani kesalahan jika terjadi
      console.error("Error updating blog:", error);
      res
        .status(500)
        .json({ error: "Terjadi kesalahan saat mengupdate data blog." });
    }
  }
);
route.delete("/delete-blog/:id", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Terjadi kesalahan saat menghapus data blog." });
  }
});
